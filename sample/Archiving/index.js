...
var opentok;
var apiKey = '47707361';
var apiSecret = '848594c1b45342c2bece08ec804d5f1372359101';

// Verify that the API Key and API Secret are defined
if (!apiKey || !apiSecret) {
  console.log(
    'You must specify API_KEY and API_SECRET environment variables'
  );
  process.exit(1);
}
  ...
  opentok.listArchives(
    ...
    function (err, archives, count) {
      if (err)
        return res.send(
          500,
          'Could not list archives. error=' + err.message
        );
      return res.render('history.ejs', {
        archives: archives,
        showPrevious: page > 1 ? '/history?page=' + (page - 1) : null,
        showNext:
          count > offset + 5 ? '/history?page=' + (page + 1) : null,
      });
    }
  );
});
  ...
  opentok.setStreamClassLists(
    ...
    function (err) {
      if (err)
        return res.send(
          500,
          'Could not set class lists. Error:' + err.message
        );
      return res.send(200, 'OK');
    }
  );
